...
def remove_duplicate_cast_nodes(model: onnx.ModelProto) -> None:
    ...
    When the same tensor is consumed by several ops on the op_block_list,
    onnxruntime.transformers.float16.convert_float_to_float16 inserts one
    identical Cast node per consumption site, all sharing the same
    autogenerated output name. This leaves the graph with multiple nodes
    producing the same output name, which violates ONNX's single static
    assignment requirement. Since these nodes have identical op type, input,
    and target dtype, keep only the first occurrence and drop the rest; all
    consumers already reference the shared name, so no rewiring is needed.
    """
    graph = model.graph
    ...
    new_nodes = [n for i, n in enumerate(graph.node) if i not in nodes_to_remove]
    del graph.node[:]
    graph.node.extend(new_nodes)

    logger.info("Removed %d duplicate Cast nodes", len(nodes_to_remove))
