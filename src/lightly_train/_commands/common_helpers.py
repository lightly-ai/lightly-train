...
def get_float32_matmul_precision(
    ...
    if float32_matmul_precision == "auto":
        ...
        if isinstance(accelerator, CUDAAccelerator):
            devices = accelerator.get_parallel_devices(devices=device_ids)
            if precision == "highest" and all(
                _device_has_tensor_cores(d) for d in devices
            ):
                device_name = torch.cuda.get_device_name(devices[0])
                # Keep this as debug for now. We should select a better default
                # precision in the future.
                logger.debug(
                    f"You are using a CUDA device with tensor cores ({device_name}). "
                    "To trade-off precision for performance, set "
                    "float32_matmul_precision to 'high' or 'medium'. The current "
                    f"default is '{precision}'. See https://docs.pytorch.org/docs/stable/generated/torch.set_float32_matmul_precision.html#torch.set_float32_matmul_precision "
...
def _device_has_tensor_cores(device: torch.device) -> bool:
    """Check if the accelerator has tensor cores."""
    if torch.cuda.is_available():
        major, _ = torch.cuda.get_device_capability(device)
    else:
        major = 0
    return device.type == "cuda" and major >= 8
