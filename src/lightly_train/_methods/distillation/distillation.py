...
class Distillation(Method):
    ...
    def __init__(
        ...
        # Check that the teacher model is supported.
        ...
        # Get the teacher model information.
        teacher_info = self._teacher_models[teacher_name]
        ...
        self.teacher_embedding_model, _, teacher_embed_dim = build_model_from_cfg(cfg)
        self.teacher_embedding_model.eval()

        # Cache directory for checkpoints
        cache_dir = Path.home() / ".cache" / "lightlytrain" / "checkpoints"
        cache_dir.mkdir(parents=True, exist_ok=True)

        # File name is derived from URL
        ...
        # Load checkpoint into teacher model
        ckpt = torch.load(checkpoint_path, map_location="cpu")
        self.teacher_embedding_model.load_state_dict(ckpt, strict=True)
...
def get_config_path(config_name: str) -> str:
    ...
    full_path = config_dir / "_modules" / "dinov2" / "configs" / config_name
