...
from lightly_train._task_models.dinov2_eomt_panoptic_segmentation.transforms import (
    DINOv2EoMTPanopticSegmentationTrainTransform,
    DINOv2EoMTPanopticSegmentationTrainTransformArgs,
    DINOv2EoMTPanopticSegmentationValTransform,
    DINOv2EoMTPanopticSegmentationValTransformArgs,
)
...
class DINOv2EoMTPanopticSegmentationTrainArgs(TrainModelArgs):
...
class DINOv2EoMTPanopticSegmentationTrain(TrainModel):
    task = "panoptic_segmentation"
    train_model_args_cls = DINOv2EoMTPanopticSegmentationTrainArgs
    task_metric_args_cls = PanopticSegmentationTaskMetricArgs
    task_model_cls = DINOv2EoMTPanopticSegmentation
    train_transform_cls = DINOv2EoMTPanopticSegmentationTrainTransform
    val_transform_cls = DINOv2EoMTPanopticSegmentationValTransform

    def __init__(
        self,
        *,
        model_name: str,
        model_args: DINOv2EoMTPanopticSegmentationTrainArgs,
        data_args: MaskPanopticSegmentationDataArgs,
        train_transform_args: DINOv2EoMTPanopticSegmentationTrainTransformArgs,
        ...
        # dependency.
        from lightly_train._task_models.dinov2_eomt_panoptic_segmentation.mask_loss import (
        ...
        self.model = DINOv2EoMTPanopticSegmentation(
    ...
    def training_step(
        ...
        # Metrics
        self.train_metrics.update_loss({"loss": loss.detach()}, weight=B)  # type: ignore[operator]
    ...
    def validation_step(
        ...
        # Metrics
        self.val_metrics.update_loss({"loss": loss.detach()}, weight=len(images))  # type: ignore[operator]
        # Final layer only
