...
class DPTHead(Module):
    """DPT-style depth prediction head used by Depth Anything V3."""
    ...
    def __init__(
        ...
        *,
        ...
        self.projects = ModuleList(
            [Conv2d(dim_in, oc, kernel_size=1) for oc in out_channels]
        )
        ...
        head_features_2 = 32
        self.scratch.output_conv1 = Conv2d(
            head_features_1,
            head_features_1 // 2,
            kernel_size=3,
            stride=1,
        ...
        self.scratch.output_conv2 = Sequential(
            Conv2d(
                head_features_1 // 2,
                head_features_2,
                kernel_size=3,
            ...
            ReLU(inplace=True),
            Conv2d(head_features_2, output_dim, kernel_size=1),
        )
        if self.use_sky_head:
            self.scratch.sky_output_conv2 = Sequential(
                Conv2d(
                    head_features_1 // 2,
                    head_features_2,
                    kernel_size=3,
                ...
                ReLU(inplace=True),
                Conv2d(head_features_2, 1, kernel_size=1),
            )
    ...
    def forward(
        ...
        if self.has_conf:
            ...
            out[self.head_main] = pred.squeeze(-1).unsqueeze(1)
            out[f"{self.head_main}_conf"] = conf.unsqueeze(1)
        else:
            out[self.head_main] = _apply_activation(main_logits, self.activation)

        if self.use_sky_head:
            sky_logits = self.scratch.sky_output_conv2(fused)
            out[self.sky_name] = _apply_activation(sky_logits, self.sky_activation)
        return out
...
class _ResidualConvUnit(Module):
    def __init__(self, features: int) -> None:
        super().__init__()
        self.activation = ReLU(inplace=False)
        self.conv1 = Conv2d(features, features, kernel_size=3, padding=1)
        self.conv2 = Conv2d(features, features, kernel_size=3, padding=1)
...
class _FeatureFusionBlock(Module):
    def __init__(self, features: int, has_residual: bool = True) -> None:
        ...
        self.out_conv = Conv2d(features, features, kernel_size=1)
...
def _make_scratch(in_shape: Sequence[int], out_shape: int) -> Module:
    scratch = Module()
    scratch.layer1_rn = Conv2d(  # type: ignore[attr-defined]
        in_shape[0], out_shape, kernel_size=3, padding=1, bias=False
    )
    scratch.layer2_rn = Conv2d(  # type: ignore[attr-defined]
        in_shape[1], out_shape, kernel_size=3, padding=1, bias=False
    )
    scratch.layer3_rn = Conv2d(  # type: ignore[attr-defined]
        in_shape[2], out_shape, kernel_size=3, padding=1, bias=False
    )
    scratch.layer4_rn = Conv2d(  # type: ignore[attr-defined]
