...
class ECViTBackboneWrapper(Module):
    """Thin adapter that exposes an :class:`ECViTModelWrapper` to the LTDETR task model.
    ...
    :class:`ECViTModelWrapper` already loads its own backbone weights via
    :meth:`ECViTModelWrapper._load_backbone_weights` during construction.
    """

    # ECViT uses a ConvPyramidPatchEmbed with a fixed patch size of 16 (see
    # ``ECViTModelWrapper.__init__``); the wrapper itself does not expose a true
    # ``patch_size`` attribute, so we hard-code 16 here.
    _ECVIT_PATCH_SIZE = 16

    def __init__(self, model_wrapper: ECViTModelWrapper) -> None:
        super().__init__()
