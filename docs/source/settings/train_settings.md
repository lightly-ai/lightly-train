...
| [`model`](#model)                               | `str`<br>`Path`               | —              | Model identifier (e.g. `"ltdetrv2-s"`) or path to a local checkpoint to fine-tune from.                                                                             |
...
Model identifier (for example `"ltdetrv2-s"`) or the path to a checkpoint or exported
model file. LightlyTrain automatically downloads weights if needed.
...
lightly_train.train_object_detection(
    ...,
    model="ltdetrv2-s",  # Loads built-in fine-tuned model.
    model_args={
