...
if __name__ == "__main__":
    lightly_train.train(
        ...
        method_args={                       # Override the default teacher model
            "teacher": "dinov2/vitl14-pretrain",
        },
    )
```
````
...
````{tab} Command Line
```bash
lightly-train train out="out/my_experiment" data="my_data_dir" model="torchvision/resnet18" method="distillation" method_args.teacher="dinov2/vitl14-pretrain"
```
````
