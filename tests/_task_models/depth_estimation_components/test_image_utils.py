...
def test__da2_target_size__lower_bound_rounds_to_multiple_of_14() -> None:
    ...
    new_h, new_w = image_utils._da2_target_size(h=480, w=640, input_size=518, patch=14)
    assert (new_h, new_w) == (518, 686)


def test__da2_target_size__square_maps_to_input_size() -> None:
    new_h, new_w = image_utils._da2_target_size(h=500, w=500, input_size=518, patch=14)
    assert (new_h, new_w) == (518, 518)


def test__da2_target_size__ceils_when_below_min_val() -> None:
    ...
    new_h, new_w = image_utils._da2_target_size(h=500, w=500, input_size=520, patch=14)
    assert (new_h, new_w) == (532, 532)


def test__resize_cubic_no_round__matches_cv2_float64() -> None:
    generator = torch.Generator().manual_seed(0)
    img = torch.randint(0, 256, (3, 30, 48), generator=generator, dtype=torch.uint8)
    img64 = img.to(torch.float64)

    out = image_utils._resize_cubic_no_round(img64, new_h=42, new_w=70)
...
def test_process_image_da2__matches_official_cubic_pipeline() -> None:
    generator = torch.Generator().manual_seed(0)
    img = torch.randint(0, 256, (3, 360, 540), generator=generator, dtype=torch.uint8)

    out = image_utils.process_image_da2(img, input_size=518, patch=14)

    new_h, new_w = image_utils._da2_target_size(h=360, w=540, input_size=518, patch=14)
    ...
    # test__resize_cubic_no_round__matches_cv2_float64 on the cv2 float32 internals.
