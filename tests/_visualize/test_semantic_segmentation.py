...
def test_plot_semantic_segmentation_predictions__grid_caps_at_max_images() -> None:
    batch = _make_batch(batch_size=4, height=16, width=16)
    logits = [torch.zeros(2, 16, 16) for _ in range(4)]
...
def test_plot_semantic_segmentation_predictions__no_image_normalize_skips_denormalization() -> (
    None
):
    # Without image_normalize the image tensor passes through unchanged.
    # alpha=0 disables the overlay so 0.4 -> 102 is preserved.
    batch = _make_batch(
        image=torch.full((1, 3, 32, 32), 0.4),
        mask=torch.zeros(1, 32, 32, dtype=torch.long),
    )
...
def test_plot_semantic_segmentation_predictions__image_normalize_denormalizes() -> None:
    # Input 0.0 with mean=0.5, std=0.5 -> denormalized to 0.5 -> pixel 127.
    batch = _make_batch(
        image=torch.zeros(1, 3, 32, 32),
        mask=torch.zeros(1, 32, 32, dtype=torch.long),
